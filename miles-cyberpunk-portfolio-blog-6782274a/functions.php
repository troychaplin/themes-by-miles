 ...
 * Theme setup
 */
function miles_cyberpunk_portfolio_blog_6782274a_setup() {
...
add_action( 'after_setup_theme', 'miles_cyberpunk_portfolio_blog_6782274a_setup' );
 ...
 */
function miles_cyberpunk_portfolio_blog_6782274a_enqueue_frontend_styles() {
    ...
    wp_enqueue_style(
        'miles-cyberpunk-portfolio-blog-6782274a-styles',
        get_template_directory_uri() . '/assets/css/styles.css',
        array(),
        $styles_version
    );
}
add_action( 'wp_enqueue_scripts', 'miles_cyberpunk_portfolio_blog_6782274a_enqueue_frontend_styles' );

/**
 * Enqueue theme scripts for frontend only
 */
function miles_cyberpunk_portfolio_blog_6782274a_enqueue_scripts() {
    ...
    if ( file_exists( $script_path ) ) {
        wp_enqueue_script(
            'miles-cyberpunk-portfolio-blog-6782274a-script',
...
add_action( 'wp_enqueue_scripts', 'miles_cyberpunk_portfolio_blog_6782274a_enqueue_scripts' );

/**
 * Remove title support from pages for cleaner editing experience.
 * The homepage content provides its own visual header.
 */
function miles_cyberpunk_portfolio_blog_6782274a_customize_pages() {
    remove_post_type_support( 'page', 'title' );
}
add_action( 'init', 'miles_cyberpunk_portfolio_blog_6782274a_customize_pages' );

/**
 * Add JS class to html element for animation system.
 * Outputs early in head to prevent FOUC on animated elements.
 */
function miles_cyberpunk_portfolio_blog_6782274a_add_js_class() {
    echo '<script>document.documentElement.classList.add("js");</script>';
}
add_action( 'wp_head', 'miles_cyberpunk_portfolio_blog_6782274a_add_js_class', 1 );
