 ...
 * Theme setup
 */
function miles_super_mario_blog_theme_a840f72e_setup() {
...
add_action( 'after_setup_theme', 'miles_super_mario_blog_theme_a840f72e_setup' );
 ...
 */
function miles_super_mario_blog_theme_a840f72e_enqueue_frontend_styles() {
    ...
    wp_enqueue_style(
        'miles-super-mario-blog-theme-a840f72e-styles',
        get_template_directory_uri() . '/assets/css/styles.css',
        array(),
        $styles_version
    );
}
add_action( 'wp_enqueue_scripts', 'miles_super_mario_blog_theme_a840f72e_enqueue_frontend_styles' );

/**
 * Enqueue theme scripts for frontend only
 */
function miles_super_mario_blog_theme_a840f72e_enqueue_scripts() {
    ...
    if ( file_exists( $script_path ) ) {
        wp_enqueue_script(
            'miles-super-mario-blog-theme-a840f72e-script',
...
add_action( 'wp_enqueue_scripts', 'miles_super_mario_blog_theme_a840f72e_enqueue_scripts' );

/**
 * Remove title support from pages for cleaner editing experience.
 * The homepage content provides its own visual header.
 */
function miles_super_mario_blog_theme_a840f72e_customize_pages() {
    remove_post_type_support( 'page', 'title' );
}
add_action( 'init', 'miles_super_mario_blog_theme_a840f72e_customize_pages' );

/**
 * Add JS class to html element for animation system.
 * Outputs early in head to prevent FOUC on animated elements.
 */
function miles_super_mario_blog_theme_a840f72e_add_js_class() {
    echo '<script>document.documentElement.classList.add("js");</script>';
}
add_action( 'wp_head', 'miles_super_mario_blog_theme_a840f72e_add_js_class', 1 );
