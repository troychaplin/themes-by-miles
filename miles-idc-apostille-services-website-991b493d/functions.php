 ...
 * Theme setup
 */
function miles_idc_apostille_services_website_991b493d_setup() {
    // Enable editor styles support
    add_theme_support( 'editor-styles' );
}
add_action( 'after_setup_theme', 'miles_idc_apostille_services_website_991b493d_setup' );
 ...
 */
function miles_idc_apostille_services_website_991b493d_enqueue_block_assets() {
    ...
    wp_enqueue_style(
        'miles-idc-apostille-services-website-991b493d-styles',
    ...
    wp_enqueue_style(
        'miles-idc-apostille-services-website-991b493d-editor-page-styles',
        get_template_directory_uri() . '/assets/css/editor-page.css',
        array( 'miles-idc-apostille-services-website-991b493d-styles' ),
        $editor_version
    );
}
add_action( 'enqueue_block_assets', 'miles_idc_apostille_services_website_991b493d_enqueue_block_assets' );

/**
 * Enqueue theme scripts for frontend only
 */
function miles_idc_apostille_services_website_991b493d_enqueue_scripts() {
    ...
    if ( file_exists( $script_path ) ) {
        wp_enqueue_script(
            'miles-idc-apostille-services-website-991b493d-script',
...
add_action( 'wp_enqueue_scripts', 'miles_idc_apostille_services_website_991b493d_enqueue_scripts' );

/**
 * Remove title support from pages for cleaner editing experience.
 * The homepage content provides its own visual header.
 */
function miles_idc_apostille_services_website_991b493d_customize_pages() {
    remove_post_type_support( 'page', 'title' );
}
add_action( 'init', 'miles_idc_apostille_services_website_991b493d_customize_pages' );

/**
 * Add JS class to html element for animation system.
 * Outputs early in head to prevent FOUC on animated elements.
 */
function miles_idc_apostille_services_website_991b493d_add_js_class() {
    echo '<script>document.documentElement.classList.add("js");</script>';
}
add_action( 'wp_head', 'miles_idc_apostille_services_website_991b493d_add_js_class', 1 );
